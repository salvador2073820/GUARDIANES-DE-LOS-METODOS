...
def nivel11(pantalla, ancho, alto):
    ...
    plataformas = [
        ...
        pygame.Rect(650, alto - ALTURA_SUELO - 110, 240, 25),
        pygame.Rect(1100, alto - ALTURA_SUELO - 250, 260, 25),
        pygame.Rect(1600, alto - ALTURA_SUELO - 160, 230, 25),
        pygame.Rect(2000, alto - ALTURA_SUELO - 260, 260, 25),
        pygame.Rect(2450, alto - ALTURA_SUELO - 150, 220, 25),
        pygame.Rect(2850, alto - ALTURA_SUELO - 240, 260, 25),
        pygame.Rect(3300, alto - ALTURA_SUELO - 100, 230, 25),
        pygame.Rect(3750, alto - ALTURA_SUELO - 200, 260, 25),
        pygame.Rect(4150, alto - ALTURA_SUELO - 80, 200, 25),
    ]
    ...
    try:
        ...
        portal_img = pygame.transform.scale(portal_img, (270, 360))
    except:
        portal_img = pygame.Surface((200, 260), pygame.SRCALPHA)
        portal_img.fill((100, 50, 200, 160))

    meta = pygame.Rect(3600, alto - ALTURA_SUELO - 360, 270, 360)
    ...
    class ObjetoRojo:
        ...
        def dibujar(self, surf, camara_x):
            pygame.draw.rect(surf, (255, 0, 0), self.rect.move(-camara_x, 0), border_radius=6)
            pygame.draw.circle(surf, (255, 180, 180),
                               self.rect.move(-camara_x, 0).center, 10)

        def intentar_danar(self, jugador_rect):
            if self.rect.colliderect(jugador_rect) and self.cooldown == 0:
                self.cooldown = 60
                return True
            return False

    objetos_rojos = [
        ObjetoRojo(550, alto - ALTURA_SUELO - 130),
        ObjetoRojo(1250, alto - ALTURA_SUELO - 280),
        ObjetoRojo(1950, alto - ALTURA_SUELO - 60),
        ObjetoRojo(2600, alto - ALTURA_SUELO - 200),
        ObjetoRojo(3100, alto - ALTURA_SUELO - 100),
    ]

    class Enemigo:
        def __init__(self, x, y, rango=180, vel=2):
        ...
        def actualizar(self):
            ...
            if abs(self.rect.x - self.orig) >= self.rango:
                self.dir *= -1

            if self.cool > 0:
                self.cool -= 1

        def dibujar(self, surf, cam_x):
            pygame.draw.rect(surf, (190, 40, 40), self.rect.move(-cam_x, 0))

        def intentar_danar(self, jugador_rect):
            if self.rect.colliderect(jugador_rect) and self.cool == 0:
                self.cool = 60
                return True
            return False

    enemigos = [
        Enemigo(900, alto - ALTURA_SUELO - 60, 200, 3),
        Enemigo(1800, alto - ALTURA_SUELO - 150, 160, 2),
        Enemigo(3200, alto - ALTURA_SUELO - 60, 240, 3),
    ]
    ...
    MENSAJES_ALEATORIOS = [
        {
            ...
            "titulo": "MÉTODO SECANTE",
            "texto": [
                "Usando el método Secante para",
                "f(x)=e^4x - 7",
                "Contesta lo siguiente:"
            ],
            "inputs": [
                {"label": "Valor de x9", "correct_answer": "0.485925583", "placeholder": "x9=?"},
                {"label": "Margen de error cuando i=7", "correct_answer": "0.132925419", "placeholder": "Respuesta"},
                {"label": "Valor de x cuando margen de error = 0.00056225", "correct_answer": "0.486485833", "placeholder": "Respuesta"}
            ]
        },
        {
            ...
            "inputs": [
                {"label": "Valor de x3", "correct_answer": "-0.269230769", "placeholder": "x3=?"},
                {"label": "Margen de error cuando i=4", "correct_answer": "0.034538632", "placeholder": "Respuesta"},
                {"label": "Valor de x cuando margen de error = 0.00100309348", "correct_answer": "-0.302766307", "placeholder": "Respuesta"}
            ]
        },
        {
            ...
            "inputs": [
                 {"label": "Valor de x2", "correct_answer": "0.612699836", "placeholder": "x2=?"},
                {"label": "Margen de error cuando i=3", "correct_answer": "0.048861447", "placeholder": "Respuesta"},
                {"label": "Valor de x cuando margen de error = 0.000027051", "correct_answer": "0.567143306", "placeholder": "Respuesta"}
            ]
        }
    ]
    ...
    while True:
                        ...
                        if e.key in (pygame.K_RETURN, pygame.K_SPACE, pygame.K_ESCAPE):
                            ...
                            if check_result is True:
                                ...
                                try:
                                    ...
                                    print("Error al cargar Nivel 1:", ex)
        ...
        texto = fuente.render("NIVEL 11 - MÉTODO SECANTE", True, (255, 255, 255))
        ...
        if mostrar_mensaje and (show_overlay or tiempo_agotado_overlay):
            ...
            if tiempo_agotado_overlay:
                ...
                mensaje_hint = "ENTER para avanzar"
            else:
                color_fondo = (200, 50, 50)
                color_texto = (255, 255, 255)
                if fallos_en_problema_actual >= 3:
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("NIVEL 11 - MÉTODO SECANTE")
