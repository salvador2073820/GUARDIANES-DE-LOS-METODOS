...
def nivel7(pantalla, ancho, alto):
    """Nivel 7 del Mundo 4 - Runge-Kutta de 4to orden por 3/8 de Simpson"""
    ...
    MENSAJES_ALEATORIOS = [
        {
            ...
            "titulo": "RUNGE-KUTTA 4TO ORDEN - 3/8 SIMPSON",
            "ecuacion": "y' = 5y - 1",
            "condiciones": ["y(0) = 2", "h = 0.2"],
            "texto": ["Usa el método de Runge-Kutta de 4to orden", "por 3/8 de Simpson"],
            "inputs": [
                {"label": "Y1 =", "correct_answer": "1.991958025", "placeholder": "y(0.2)"},
                {"label": "Y2 =", "correct_answer": "2.437679904", "placeholder": "y(0.4)"}
            ]
        },
        {
            ...
            "titulo": "RUNGE-KUTTA 4TO ORDEN - 3/8 SIMPSON",
            "ecuacion": "y' = y² + t - y",
            "condiciones": ["y(0) = 1", "h = 0.5"],
            "texto": ["Usa el método de Runge-Kutta de 4to orden", "por 3/8 de Simpson"],
            "inputs": [
                {"label": "Y1 =", "correct_answer": "0.552579997", "placeholder": "y(0.5)"},
                {"label": "Y2 =", "correct_answer": "0.328646044", "placeholder": "y(1.0)"}
            ]
        },
        {
            ...
            "titulo": "RUNGE-KUTTA 4TO ORDEN - 3/8 SIMPSON", 
            "ecuacion": "y' = t² - y",
            "condiciones": ["y(0) = 1", "h = 0.1"],
            "texto": ["Usa el método de Runge-Kutta de 4to orden", "por 3/8 de Simpson"],
            "inputs": [
                {"label": "Y1 =", "correct_answer": "0.9051625", "placeholder": "y(0.1)"},
                {"label": "Y2 =", "correct_answer": "0.8212692", "placeholder": "y(0.2)"}
            ]
        }
    ]
    ...
    def cargar_nivel_siguiente():
        """Cargar el nivel 8 después de completar este nivel"""
        try:
            directorio_actual = os.path.dirname(os.path.abspath(__file__))
            ruta_nivel8 = os.path.join(directorio_actual, "nivel8.py")
            print(f"[nivel7] intentando cargar: {ruta_nivel8}")

            if os.path.exists(ruta_nivel8):
                ...
                import nivel8
                importlib.reload(nivel8)

                # Llamar al nivel 8 (no cerramos pygame aquí)
                nivel8.nivel8(pantalla, ancho, alto)
                return True
            else:
                print(f"[nivel7] nivel8.py no encontrado en: {directorio_actual}")
                return False
        except Exception as e:
            print(f"[nivel7] Error al cargar nivel8: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    while True:
                    ...
                    if show_overlay:
                        if e.key in (pygame.K_RETURN, pygame.K_SPACE, pygame.K_ESCAPE):
                            ...
                            if check_result is True:
                                ...
                                # Mensaje de consola y cargar nivel 8
                                print("[nivel7] ¡Respuesta correcta! Ganaste 1 vida. Cargando nivel 8...")
                                if cargar_nivel_siguiente():
                                    # Si nivel8 se ejecuta, salimos del bucle de este nivel.
                                    return
                                else:
                                    print("[nivel7] No se pudo cargar nivel8.py - volviendo al menú o reiniciando.")
                                    return
        ...
        # HUD
        texto = fuente.render("MUNDO 4 - NIVEL 7", True, (255, 255, 255))
        ...
        if mostrar_mensaje and (show_overlay or tiempo_agotado_overlay):
            ...
            if tiempo_agotado_overlay:
                ...
                mensaje_hint = "Presiona ENTER para continuar al NIVEL 8"
            else:
                color_fondo = (200, 50, 50)
                color_texto = (255, 255, 255)
                if fallos_en_problema_actual >= 3:
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption("MUNDO 4 - NIVEL 7 - RUNGE-KUTTA 4TO ORDEN - 3/8 SIMPSON")
