...
def nivel4(pantalla, ancho, alto):
    """Nivel 4 del Mundo 4 - Runge-Kutta 2do Orden"""
    ...
    # ---------- Obstáculos personalizados para Nivel 4 (Diseño Estratégico Nuevo) ----------
    def generar_obstaculos():
        ...
        plataformas_local.extend([
            pygame.Rect(inicio_offset + 60, alto - ALTURA_SUELO - 100, 220, 20),
            pygame.Rect(inicio_offset + 400, alto - ALTURA_SUELO - 170, 200, 20),
            pygame.Rect(inicio_offset + 780, alto - ALTURA_SUELO - 130, 180, 20),
            pygame.Rect(inicio_offset + 1100, alto - ALTURA_SUELO - 90, 160, 20),
        ])

        objetos_local.extend([
            ObjetoRojo(inicio_offset + 300, alto - ALTURA_SUELO - 40),
            ObjetoRojo(inicio_offset + 850, alto - ALTURA_SUELO - 40),
        ])

        enemigos_local.append(
            Enemigo(inicio_offset + 640, alto - ALTURA_SUELO - 160, rango=140, velocidad=2)
        )

        # ------ Zona media: puentes grandes ------
        medio_offset = 1700
        plataformas_local.extend([
            pygame.Rect(medio_offset + 0, alto - ALTURA_SUELO - 100, 300, 20),
            pygame.Rect(medio_offset + 380, alto - ALTURA_SUELO - 200, 260, 20),
            pygame.Rect(medio_offset + 800, alto - ALTURA_SUELO - 140, 260, 20),
            pygame.Rect(medio_offset + 1180, alto - ALTURA_SUELO - 230, 300, 20),
        ])

        objetos_local.extend([
            ObjetoRojo(medio_offset + 200, alto - ALTURA_SUELO - 40),
            ObjetoRojo(medio_offset + 700, alto - ALTURA_SUELO - 40),
            ObjetoRojo(medio_offset + 1350, alto - ALTURA_SUELO - 40),
        ])
        ...
        enemigos_local.append(
            Enemigo(medio_offset + 1300, alto - ALTURA_SUELO - 240, rango=180, velocidad=2)
        )

        # ------ Zona final: escaleras hacia el portal ------
        final_offset = 3050
        plataformas_local.extend([
            pygame.Rect(final_offset + 40, alto - ALTURA_SUELO - 100, 240, 20),
            pygame.Rect(final_offset + 380, alto - ALTURA_SUELO - 160, 220, 20),
            pygame.Rect(final_offset + 720, alto - ALTURA_SUELO - 210, 250, 20),
            pygame.Rect(final_offset + 1080, alto - ALTURA_SUELO - 170, 220, 20),
            pygame.Rect(final_offset + 1400, alto - ALTURA_SUELO - 130, 200, 20),
        ])

        objetos_local.extend([
            ObjetoRojo(final_offset + 200, alto - ALTURA_SUELO - 40),
            ObjetoRojo(final_offset + 620, alto - ALTURA_SUELO - 40),
            ObjetoRojo(final_offset + 1130, alto - ALTURA_SUELO - 40),
        ])

        enemigos_local.append(
            Enemigo(final_offset + 880, alto - ALTURA_SUELO - 200, rango=160, velocidad=3)
        )
    ...
    MENSAJES_ALEATORIOS = [
        {
            ...
            "titulo": "RUNGE-KUTTA 2do ORDEN",
            "ecuacion": "y' - 2yt + 3 = 0",
           "condiciones": ["y0 = 1, h = 0.1"],
            "texto": ["Calcula y1, y2 usando el método de Runge-Kutta 2do orden."],
            "inputs": [
                {"label": "k1 para y1 =", "correct_answer": "-0.3", "placeholder": "Respuesta"},
                {"label": "y1 =", "correct_answer": "0.707", "placeholder": "Respuesta"},
                {"label": "k2 para y2=", "correct_answer": "-0.2831544", "placeholder": "Respuesta"},
                {"label": "y2 =", "correct_answer": "0.4224928", "placeholder": "Respuesta"}
            ]
        },
        {
            ...
            "titulo": "RUNGE-KUTTA 2do ORDEN",
            "ecuacion": "y' - yt - 4 = 0",
           "condiciones": ["y0 = 3, h = 0.1"],
            "texto": ["Calcula y1, y2 usando el método de Runge-Kutta 2do orden."],
            "inputs": [
                {"label": "k2 para y1 =", "correct_answer": "0.366", "placeholder": "Respuesta"},
                {"label": "y1 =", "correct_answer": "3.383", "placeholder": "Respuesta"},
                {"label": "k1 para y2=", "correct_answer": "0.36617", "placeholder": "Respuesta"},
                {"label": "y2 =", "correct_answer": "3.7285933", "placeholder": "Respuesta"}
            ]
        },
        {
           ...
           "condiciones": ["y0 = 2, h = 0.2"],
            ...
            "inputs": [
                ...
                {"label": "y1 =", "correct_answer": "1.98", "placeholder": "Respuesta"},
                {"label": "k2 para y2=", "correct_answer": "0.6704", "placeholder": "Respuesta"},
                {"label": "y2 =", "correct_answer": "2.4132", "placeholder": "Respuesta"}
            ]
        }
    ]
    ...
    def cargar_nivel5():
        try:
            ...
            ruta_nivel5 = os.path.join(directorio_actual, "nivel5.py")
            print(f"[nivel4] intentando cargar: {ruta_nivel5}")
            ...
            if os.path.exists(ruta_nivel5):
                ...
                import nivel5
                importlib.reload(nivel5)

                nivel5.nivel5(pantalla, ancho, alto)
                return True
            else:
                print(f"[nivel4] nivel5.py no encontrado en: {directorio_actual}")
                return False
        except Exception as e:
            print(f"[nivel4] Error al cargar nivel5: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    while True:
                    ...
                    if show_overlay:
                        if e.key in (pygame.K_RETURN, pygame.K_SPACE, pygame.K_ESCAPE):
                            ...
                            if check_result is True:
                                ...
                                print("[nivel4] ¡Respuesta correcta! Ganaste 1 vida. Cargando nivel 5...")
                                if cargar_nivel5():
                                    return
                                else:
                                    print("[nivel4] No se pudo cargar nivel5.py")
                                    return
        ...
        if mostrar_mensaje and (show_overlay or tiempo_agotado_overlay):
            ...
            if tiempo_agotado_overlay:
                ...
                mensaje_hint = "Presiona ENTER para continuar al NIVEL 5"
            else:
                color_fondo = (200, 50, 50)
                color_texto = (255, 255, 255)
                if fallos_en_problema_actual >= 3:
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption("MUNDO 4 - NIVEL 4 - RUNGE-KUTTA 2do ORDEN")
