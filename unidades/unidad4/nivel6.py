...
def nivel6(pantalla, ancho, alto):
    """Nivel 6 del Mundo 4 - Runge-Kutta 4to Orden (1/3 Simpson)"""
 ...
 # ---------- Obstáculos personalizados (Nivel 6 - Runge-Kutta 4to Orden + Simpson 1/3) ----------
    def generar_obstaculos():
        ...
        # ==========================
        #       ZONA 1 – INICIO
        #   (Introducción de ritmo)
        # ==========================
        inicio_offset = 150

        plataformas_local.extend([
            pygame.Rect(inicio_offset + 80, alto - ALTURA_SUELO - 110, 240, 20),
            pygame.Rect(inicio_offset + 420, alto - ALTURA_SUELO - 170, 200, 20),
            pygame.Rect(inicio_offset + 740, alto - ALTURA_SUELO - 130, 190, 20),
            pygame.Rect(inicio_offset + 1060, alto - ALTURA_SUELO - 90, 180, 20),
        ])
        ...
        enemigos_local.append(
            Enemigo(inicio_offset + 620, alto - ALTURA_SUELO - 150, rango=120, velocidad=2)
        )

        # ==========================
        #     ZONA 2 – SECCIÓN MEDIA 
        #      Plataformas trianguladas
        # ==========================
        medio_offset = 1850

        plataformas_local.extend([
            pygame.Rect(medio_offset + 40, alto - ALTURA_SUELO - 130, 220, 20),
            pygame.Rect(medio_offset + 360, alto - ALTURA_SUELO - 200, 250, 20),
            pygame.Rect(medio_offset + 720, alto - ALTURA_SUELO - 250, 260, 20),
            pygame.Rect(medio_offset + 1090, alto - ALTURA_SUELO - 200, 240, 20),
            pygame.Rect(medio_offset + 1410, alto - ALTURA_SUELO - 150, 260, 20),
        ])

        objetos_local.extend([
            ObjetoRojo(medio_offset + 250, alto - ALTURA_SUELO - 40),
            ObjetoRojo(medio_offset + 840, alto - ALTURA_SUELO - 40),
            ObjetoRojo(medio_offset + 1320, alto - ALTURA_SUELO - 40),
        ])

        enemigos_local.extend([
            Enemigo(medio_offset + 500, alto - ALTURA_SUELO - 200, rango=150, velocidad=3),
            Enemigo(medio_offset + 1150, alto - ALTURA_SUELO - 230, rango=170, velocidad=2),
            Enemigo(medio_offset + 1460, alto - ALTURA_SUELO - 150, rango=160, velocidad=3),
        ])

        # ==========================
        #     ZONA 3 – ESCALERA FINAL
        #     (subiendo hacia el portal)
        # ==========================
        final_offset = 3300

        plataformas_local.extend([
            pygame.Rect(final_offset + 60, alto - ALTURA_SUELO - 120, 260, 20),
            pygame.Rect(final_offset + 440, alto - ALTURA_SUELO - 180, 230, 20),
            pygame.Rect(final_offset + 820, alto - ALTURA_SUELO - 240, 250, 20),
            pygame.Rect(final_offset + 1180, alto - ALTURA_SUELO - 200, 260, 20),
            pygame.Rect(final_offset + 1500, alto - ALTURA_SUELO - 140, 260, 20),
        ])

        objetos_local.extend([
            ObjetoRojo(final_offset + 260, alto - ALTURA_SUELO - 40),
            ObjetoRojo(final_offset + 680, alto - ALTURA_SUELO - 40),
            ObjetoRojo(final_offset + 1110, alto - ALTURA_SUELO - 40),
        ])

        enemigos_local.append(
            Enemigo(final_offset + 920, alto - ALTURA_SUELO - 230, rango=150, velocidad=3)
        )
    ...
    MENSAJES_ALEATORIOS = [
        {
            ...
            "titulo": "RUNGE-KUTTA 4to ORDEN (1/3 Simpson)",
            "ecuacion": "y' = 5yt - 1",
           "condiciones": ["y0 = 2, h = 0.2"],
            "texto": ["Calcula y1, y2 usando el método de Runge-Kutta 4to orden (1/3 Simpson)."],
            "inputs": [
                {"label": "k3 para y1 =", "correct_answer": "-0.0005", "placeholder": "Respuesta"},
                {"label": "y1 =", "correct_answer": "1.996483333", "placeholder": "Respuesta"},
                {"label": "k4 para y2=", "correct_answer": "0.7839017032", "placeholder": "Respuesta"},
                {"label": "y2 =", "correct_answer": "2.45771987", "placeholder": "Respuesta"}
            ]
        },
        {
            "problema_titulo": "PROBLEMA 2",
            "titulo": "RUNGE-KUTTA 4to ORDEN (1/3 Simpson)",
            "ecuacion": "y'= (y + t)^2 / 1 - y",
           "condiciones": ["y0 = 0.4, h = 0.2"],
            "texto": ["Calcula y1, y2 usando el método de Runge-Kutta 4to orden (1/3 Simpson)."],
            "inputs": [
                {"label": "k4 para y1 =", "correct_answer": "0.2047895890", "placeholder": "Respuesta"},
                {"label": "y1 =", "correct_answer": "0.511617621", "placeholder": "Respuesta"},
                {"label": "k3 para y2=", "correct_answer": "0.7831875201", "placeholder": "Respuesta"},
                {"label": "y2 =", "correct_answer": "0.6276546544", "placeholder": "Respuesta"}
            ]
        }
    ]
    ...
    def cargar_nivel7():
        try:
            ...
            ruta_nivel7 = os.path.join(directorio_actual, "nivel7.py")
            print(f"[nivel6] intentando cargar: {ruta_nivel7}")

            if os.path.exists(ruta_nivel7):
                if directorio_actual not in sys.path:
                    sys.path.append(directorio_actual)

                import nivel7
                importlib.reload(nivel7)

                nivel7.nivel7(pantalla, ancho, alto)
                return True
            else:
                print(f"[nivel6] nivel7.py no encontrado en: {directorio_actual}")
                return False
        except Exception as e:
            print(f"[nivel6] Error al cargar nivel7: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    while True:
                    ...
                    if show_overlay:
                        if e.key in (pygame.K_RETURN, pygame.K_SPACE, pygame.K_ESCAPE):
                            ...
                            if check_result is True:
                                ...
                                print("[nivel6] ¡Respuesta correcta! Ganaste 1 vida. Cargando nivel 7...")
                                if cargar_nivel7():
                                    return
                                else:
                                    print("[nivel6] No se pudo cargar nivel7.py")
                                    return
        ...
        if mostrar_mensaje and (show_overlay or tiempo_agotado_overlay):
            ...
            if tiempo_agotado_overlay:
                ...
                mensaje_hint = "Presiona ENTER para continuar al NIVEL 7"
            else:
                color_fondo = (200, 50, 50)
                color_texto = (255, 255, 255)
                if fallos_en_problema_actual >= 3:
